@extends('layout', ['title' => Lang::get("Sequence Period"), 'subTitle' => Lang::get("Manage data sequence period")])
                ...
                <div class="card-body">                    
                    <div class="row mb-2">
                        <div class="col-12 d-flex justify-content-between">
                            <div>
                                @can('sequence-period-create')
                                <a href="{{route('sequence.period.create', ['parentId'=>$data->id])}}" class="btn btn-primary" id="btn-add"><i class="fas fa-plus"></i> {{__('Create')}}</a>                                                
                                @endcan
                            </div>
    ...
    <script>
        $('document').ready(function(){
            $('#datatable').DataTable({
                ajax:
                {
                    url: "{{route('sequence.period.datatable', ['parentId'=>$data->id])}}",
                    type: 'POST',
                    data: function(data){
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        visible: @can('sequence-period-delete') true @else false @endcan,
                        render: function(data, type, row)
                        {
                    ...
                    },
                    {
                        ...
                        defaultContent: '',
                        className: 'text-center',
                        visible: @can('sequence-period-edit') true @else false @endcan,
                        render: function(data, type, row)
                        {
