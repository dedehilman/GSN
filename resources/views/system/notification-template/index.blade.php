@extends('layout', ['title' => Lang::get("Notification Template"), 'subTitle' => Lang::get("Manage data notification template")])
                    ...
                    <div class="row mb-2">
                        <div class="col-12 d-flex justify-content-between">
                            <div>
                                ...
                                <a href="{{route('notification-template.create')}}" class="btn btn-primary" id="btn-add"><i class="fas fa-plus"></i> {{__('Create')}}</a>                                                
                                @endcan
                            </div>
    ...
    <script>
        $('document').ready(function(){
            $('#datatable').DataTable({
                ajax:
                {
                    url: "{{route('notification-template.datatable')}}",
                    type: 'POST',
                    data: function(data){
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        visible: @can('notification-template-delete') true @else false @endcan,
                        render: function(data, type, row)
                        {
                    ...
                    },
                    {
                        ...
                        defaultContent: '',
                        className: 'text-center',
                        visible: @can('notification-template-edit') true @else false @endcan,
                        render: function(data, type, row)
                        {
                        ...
                        render: function(data, type, row)
                        {
                            ...
                            if(Array.isArray(data)) {
                                data.forEach((element, index) => {
                                    if(index == 0) {
                                        $notificationTypes = element.notification_type;
                                    } else {
                                        $notificationTypes += ', ' + element.notification_type;
                                    }
