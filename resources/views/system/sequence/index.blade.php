@extends('layout', ['title' => Lang::get("Sequence"), 'subTitle' => Lang::get("Manage data sequence")])
                ...
                <div class="card-body">                    
                    <div class="row mb-2">
                        <div class="col-12 d-flex justify-content-between">
                            <div>
                                @can('sequence-create')
                                <a href="{{route('sequence.create')}}" class="btn btn-primary" id="btn-add"><i class="fas fa-plus"></i> {{__('Create')}}</a>                                                
                                @endcan
                            </div>
                    ...
                    <table id="datatable" class="table table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th>{{ __("Name") }}</th>
                                <th>{{ __("Format") }}</th>
                                <th>{{ __("Number Increment") }}</th>
                                <th>{{ __("Number Next") }}</th>
                                <th>{{ __("Use Date Range") }}</th>
                            </tr>
                        </thead>
                    </table>
                </div>
            ...
            $('#datatable').DataTable({
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        visible: @can('sequence-delete') true @else false @endcan,
                        render: function(data, type, row)
                        {
                    ...
                    },
                    {
                        ...
                        defaultContent: '',
                        className: 'text-center',
                        visible: @can('sequence-edit') true @else false @endcan,
                        render: function(data, type, row)
                        {
