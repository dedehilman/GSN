@extends('layout', ['title' => Lang::get("Record Rule"), 'subTitle' => Lang::get("Manage data record rule")])
                ...
                <div class="card-body">                    
                    <div class="row mb-2">
                        <div class="col-12 d-flex justify-content-between">
                            <div>
                                @can('record-rule-create')
                                <a href="{{route('record-rule.create')}}" class="btn btn-primary" id="btn-add"><i class="fas fa-plus"></i> {{__('Create')}}</a>
                                @endcan
                            </div>
    ...
    <script>
        $('document').ready(function(){
            $('#datatable').DataTable({
                ajax:
                {
                    url: "{{route('record-rule.datatable')}}",
                    type: 'POST',
                    data: function(data){
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        visible: @can('record-rule-delete') true @else false @endcan,
                        render: function(data, type, row)
                        {
                    ...
                    },
                    {
                        ...
                        defaultContent: '',
                        className: 'text-center',
                        visible: @can('record-rule-edit') true @else false @endcan,
                        render: function(data, type, row)
                        {
