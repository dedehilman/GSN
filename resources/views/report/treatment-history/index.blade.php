@extends('layout', ['title' => Lang::get("Treatment History"), 'subTitle' => Lang::get("Manage data treatment history report")])
                ...
                <div class="card-body">                    
                    <div class="row mb-2">
                        <div class="col-12 d-flex justify-content-between">
                            <div>
                                {{-- @can('treatment-history-report-create') --}}
                                <a href="{{route('report.treatment-history.create')}}" class="btn btn-primary" id="btn-add"><i class="fas fa-plus"></i> {{__('Create')}}</a>
                                {{-- @endcan --}}
                            </div>
    ...
    <script>
        $('document').ready(function(){
            $('#datatable').DataTable({
                ajax:
                {
                    url: "{{route('report.treatment-history.datatable')}}",
                    type: 'POST',
                    data: function(data){
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        visible: @can('grievance-report-treatment-history-delete') true @else false @endcan,
                        render: function(data, type, row)
                        {
