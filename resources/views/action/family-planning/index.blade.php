@extends('layout', ['title' => Lang::get("Family Planning"), 'subTitle' => Lang::get("Manage data family planning")])
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-4 referenceRow d-none">
                                            <div class="input-group">
                                                ...
                                                <input type="hidden" name="family_plannings.reference_id" id="reference_id">
                                                <div class="input-group-append">
    ...
    <script>
        $('document').ready(function(){
            $('#datatable').DataTable({
                ajax:
                {
                    url: "{{route('action.family-planning.datatable')}}",
                    type: 'POST',
                    data: function(data){
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        visible: @can('action-family-planning-delete') true @else false @endcan,
                        render: function(data, type, row)
                        {
            ...
            $("select[name='family_plannings.reference_type']").on('change', function(){
                $("#reference_id").val("");
                $("#reference_name").val("");
                
                if($(this).val() == 'Internal') {
                    $(".reference-modal-select").attr("data-url", "{{route('clinic.select', 'queryBuilder=0')}}");
                    $("#reference_id").attr("name", "family_plannings.reference_clinic_id");
                    $(".referenceRow").removeClass("d-none");
                } else if($(this).val() == 'External') {
                    $(".reference-modal-select").attr("data-url", "{{route('reference.select')}}");
                    $("#reference_id").attr("name", "family_plannings.reference_id");
                    $(".referenceRow").removeClass("d-none");
                } else {
