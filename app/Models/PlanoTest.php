...
class PlanoTest extends Model implements HasMedia
{
    use HasFactory, BlameableTrait, InteractsWithMedia;

    protected $fillable = ['transaction_no', 'transaction_date', 'remark', 'clinic_id', 'medical_staff_id', 'patient_id', 'reference_type', 'reference_id', 'reference_clinic_id','for_relationship', 'patient_relationship_id', 'result', 'pregnancy_num','status'];
    ...
    public function action() {
        return Action::where('model_type', "App\Models\PlanoTest")->where("model_id", $this->id)->first();
    }

    public function pharmacy() {
        return Pharmacy::where('model_type', "App\Models\PlanoTest")->where("model_id", $this->id)->first();
    }
